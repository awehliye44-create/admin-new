 ...
 */ export function formatPenceWithCurrency(pence, currencyCode) {
 ...
 */ export function formatPenceSigned(pence, currencyCode) {
  const symbol = getCurrencySymbol(currencyCode);
  const amount = Math.abs(pence) / 100;
  const sign = pence < 0 ? '-' : '+';
  return `${sign}${symbol}${amount.toFixed(2)}`;
 ...
 */ export async function resolveDriverCurrency(supabase, driverId) {
  const { data: driver } = await supabase.from('drivers').select('region_id').eq('id', driverId).single();
  if (!driver?.region_id) return null;
  const { data: region } = await supabase.from('regions').select('currency_code').eq('id', driver.region_id).single();
  return region?.currency_code || null;
}
/**
 * Resolve currency from a driver's region_id (when you already have region_id).
 */ export async function resolveRegionCurrency(supabase, regionId) {
  if (!regionId) return null;
  const { data: region } = await supabase.from('regions').select('currency_code').eq('id', regionId).single();
  return region?.currency_code || null;
}
