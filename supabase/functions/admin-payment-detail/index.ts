...
serve(async (req) => {
  ...
  try {
    ...
    const { data: payments } = await supabase
      .from('payments')
      .select('id, provider_order_id, provider_payment_id, captured_amount_pence, amount_pence, status, metadata, capture_method, created_at, provider_charge_id')
      .eq('trip_id', tripId)
    ...
    const paymentRows = (payments ?? []).map((payment) => {
      ...
      return {
        id: payment.id,
        stripePaymentIntentId: payment.provider_order_id ?? payment.provider_payment_id ?? null,
    ...
    // ONECAB net after provider fee — read from DB (do NOT recompute on the client).
    ...
    const response = {
      ...
      stripe: {
        paymentIntentId: trip.provider_order_id ?? null,
        chargeId: null,
