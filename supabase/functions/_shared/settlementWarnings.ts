/**
 * Formatting helpers for historical settlement warnings surfaced by
...
const INFORMATIONAL_WARNINGS = new Set<string>([
  ...
  "LEGACY_STRIPE_EVIDENCE_READ_ONLY",
]);
...
export function formatSettlementWarning(warning: string | null | undefined): string | null {
  ...
  switch (warning) {
    case "SEPARATE_CHARGE_TRANSFER_USED_NO_APPLICATION_FEE_OBJECT":
      return "Legacy separate-charge-transfer settlement (historical evidence only).";
    case "LEGACY_STRIPE_EVIDENCE_READ_ONLY":
      return "Legacy Stripe trip — persisted evidence only; live Stripe API retired.";
    case "STRIPE_SETTLEMENT_NOT_VERIFIED_NO_APPLICATION_FEE_OR_TRANSFER":
      return "Legacy settlement could not be verified — no application fee or transfer recorded.";
    case "STRIPE_SETTLEMENT_UNVERIFIED":
      return "Legacy settlement is unverified.";
    default:
