/**
 * Slice 12 â controlled Revolut Business provider submission for READY company transfers.
 * REVOLUT_PAYMENT_TRANSPORT_ENABLED=true; LIVE_COMPANY_TRANSFER_EXECUTION_ENABLED=false
 * (fail-closed proof â no /pay, no hold, no debit). Company funding hold stays separate
...
export function maySubmitCompanyTransferViaTransport(env: {
  ...
  // Independent of driver LIVE_PAYOUT â company and driver money paths are isolated.
...
export function evaluateCompanyTransferSubmissionEligibility(args: {
  ...
  if (intent === "UNKNOWN") {
    return {
      ...
      message: "Prior UNKNOWN â no blind retry",
    };
  }
...
export function maskProviderId(id: string | null | undefined): string | null {
  ...
  if (!raw) return null;
  if (raw.length <= 8) return `${raw.slice(0, 2)}â¦`;
