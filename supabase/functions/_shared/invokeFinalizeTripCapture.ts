...
export async function invokeFinalizeTripCapture(args: {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // Shortfall / recovery / incremental pending are durable settlement outcomes —
      // treat as business-complete for invoke callers (do not force capture_failed).
      const durableSettlement =
        status.includes("shortfall") ||
        status.includes("recovery") ||
        status.includes("partial_capture") ||
        status.includes("additional_authorisation") ||
        status.includes("incremental") ||
        status === "capture_pending" ||
        status === "processing" ||
        status === "capture_busy";
      const businessOk = durableSettlement
        ? body?.deferred !== true
        : res.ok && !body?.error && body?.success !== false && body?.deferred !== true;
      if (businessOk) {
