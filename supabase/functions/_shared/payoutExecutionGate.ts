 ...
 * Live automated provider transfers require ADMIN_PAYOUT_PROVIDER_EXECUTION_ENABLED=true
...
export function isAdminProviderPayoutExecutionEnabled(): boolean {
  return Deno.env.get("ADMIN_PAYOUT_PROVIDER_EXECUTION_ENABLED") === "true";
}

/** Alias used by payoutRetryGuard. */
export function providerExecutionEnabled(): boolean {
  return isAdminProviderPayoutExecutionEnabled();
}
...
export const PAYOUT_EXECUTION_DISABLED_MESSAGE =
  "Automated payout execution disabled. Revolut/manual bank settlement does not require this flag; set ADMIN_PAYOUT_PROVIDER_EXECUTION_ENABLED=true only for owner-approved legacy Connect transfers.";
