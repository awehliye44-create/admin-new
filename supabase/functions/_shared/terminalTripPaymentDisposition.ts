...
export async function disposeTerminalTripPayment(
  ...
  const { data: trip, error: tripErr } = await supabase
    .from("trips")
    .select(
      "id, status, started_at, arrived_at, free_wait_expires_at, cancelled_at, cancelled_by, scheduled_at, cancellation_grace_expires_at, driver_id, confirmed_driver_id, service_area_id, vehicle_type_id, payment_provider, provider_order_id, stripe_payment_intent_id, authorised_amount_pence, cancellation_fee_pence, no_show_charge_pence, payment_status, arrival_cancellation_applied",
    )
    .eq("id", args.tripId)
    .maybeSingle();
  ...
  try {
    ...
    const after = await retrieveRevolutOrder(environment, secretKey, orderId);
