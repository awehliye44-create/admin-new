...
function isSessionValidForTrip(session: SessionRow, trip: TripRow): boolean {
  ...
  if (isCallableTripStatus(trip.status)) return true;
  if (trip.status === "completed" && session.expires_at) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const msg91CallerId = Deno.env.get("MSG91_CALLER_ID");

    if (!msg91AuthKey || !msg91CallerId) {
      console.error("[call-masking] MSG91_AUTH_KEY or MSG91_CALLER_ID not configured");
    ...
    if (body.action === "route-inbound") {
      ...
      const inboundContext = pickedTrip
        ? await loadTripCommunicationRuntimeContext(serviceClient, pickedTrip, msg91CallerId)
        : {
          settings: null,
          maskingConfig: null,
          maxCallDurationSeconds: DEFAULT_MAX_CALL_DURATION_SECONDS,
          maskingCallerId: msg91CallerId,
    ...
    const { data: trip } = await serviceClient
      .from("trips")
      .select("id, confirmed_driver_id, passenger_id, passenger_phone, status, completed_at, service_area_id")
      .eq("id", tripId)
      .single();
    ...
    if (TERMINAL_TRIP_STATUSES.has(trip.status) && trip.status !== "completed") {
    ...
    const isDriver = Boolean(driver && trip.confirmed_driver_id === driver.id);
    ...
    const commContext = await loadTripCommunicationRuntimeContext(serviceClient, trip, msg91CallerId);
    ...
    const driverId = trip.confirmed_driver_id;
    if (!driverId) {
    ...
    if (action === "create-session") {
      ...
      if (trip.status === "completed") {
        const { data: graceSession } = await serviceClient
          ...
          .eq("status", "active")
          .maybeSingle();
        if (!graceSession?.expires_at || new Date(graceSession.expires_at) <= new Date()) {
          return errorResponse("INVALID_STATE", "Call window has closed for this trip", 400);
        }
      }
    ...
    if (action === "initiate-call") {
      ...
      if (!sessionId || !canInitiateCall(trip as TripRow, (session ?? { status: "active", expires_at: sessionExpiresAt() }) as SessionRow)) {
        return errorResponse("INVALID_STATE", "Cannot call in current trip state", 400);
      }

      // Manual dial only — do NOT invoke MSG91 Click-to-Call (CTC auto-rings the caller).
      // User dials session.masked_number from the native dialer; MSG91 route-inbound bridges.
      logCallEvent("manual_dial_prepared", {
        booking_id: tripId,
        session_id: sessionId,
        caller: callerPhoneE164,
        destination: "",
        session_id_msg91: session?.msg91_request_id ?? null,
      });

      return successResponse({
        success: true,
        manual_dial_only: true,
        message: "Dial the masked number manually; call bridges when you press Call.",
