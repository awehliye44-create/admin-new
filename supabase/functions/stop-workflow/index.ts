...
/** Card trips must capture via finalize-trip-and-capture — never mark captured without Revolut. */
  ...
  try {
    ...
    switch (action) {
      ...
      case 'complete_trip': {
        ...
        const { requiresProviderSettlement } = await import("../_shared/digitalPaymentCapture.ts");
        ...
        if ((commissionableFarePence > 0 || tipAmountPence > 0) && !skipCardLedgerInStopWorkflow) {
          ...
          // Card payment_status is owned by finalize-trip-and-capture + revolut-webhook
