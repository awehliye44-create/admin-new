...
serveWithEdgeTiming("request-trip-modification", corsHeaders, async (req) => {
  ...
  try {
    ...
    if (changeType === "add_stop") {
      ...
      // Only future stops (after current navigation target) may be removed without driver approval.
      ...
      if (
        ...
        return new Response(JSON.stringify({
          error: "Cannot remove the driver's current destination without a navigation-impacting change request",
        }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
    ...
    const originLat = trip.status === "in_progress"
      ? (trip.driver_location_lat ?? trip.pickup_latitude)
      : trip.pickup_latitude;
    const originLng = trip.status === "in_progress"
      ? (trip.driver_location_lng ?? trip.pickup_longitude)
    ...
    async function calculateRouteLeg(
      ...
      return { routePayload, distance, duration };
    }
