...
Deno.serve(async (req) => {
  ...
  // Single-active-device policy (existing claim_active_device semantics).
  await supabase.from("driver_active_devices").upsert(
    {
      driver_id: driverId,
  ...
  // Soft-deactivate tokens for other installations.
  await supabase
    .from("push_tokens")
  ...
  // Keep presence hint in sync for send-driver-notification ordering.
  await supabase
  ...
  console.log(
    `[bind-driver-device-token] ok driver=${driverId} platform=${platform} installation=${installationId.slice(0, 8)}… fp=${fp}`,
  );
