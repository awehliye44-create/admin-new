...
export const EXECUTE_CONFIRMATION = "CREDIT_SAVED_TRIP_EARNING_NET";
...
export async function handleAdminRecoverCapturedTripWallet(
  req: Request,
  deps?: {
    authorize?: (req: Request) => Promise<GateResult | { ok: false; response: Response }>;
    recover?: typeof recoverCapturedTripWallet;
  },
): Promise<Response> {
  ...
  try {
    ...
    const recover = deps?.recover ?? recoverCapturedTripWallet;
    ...
    for (const key of FORBIDDEN_COHORT_KEYS) {
      if (body[key] != null) {
        return jsonResponse({
          success: false,
          error: "Cohort / date-range recovery is forbidden. Pass explicit trip_ids only.",
    ...
    if (!dryRun && body.confirm_execute !== EXECUTE_CONFIRMATION) {
      return jsonResponse({
        success: false,
        error: "Live credit requires confirm_execute=CREDIT_SAVED_TRIP_EARNING_NET",
        code: "EXECUTE_CONFIRMATION_REQUIRED",
      }, 400);
    }

    const results = [];
    for (const tripId of tripIds) {
      results.push(await recover(gate.supabase, {
        tripId,
        allowedTripIds: allowList,
        dryRun,
      }));
    }
    ...
    console.log("[admin-recover-captured-trip-wallet]", JSON.stringify(audit));
    ...
    console.error("[admin-recover-captured-trip-wallet] failed", message);
