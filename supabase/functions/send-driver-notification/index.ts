...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // NEVER use presence.app_state to suppress sending.
    ...
    // Do NOT read app_state here for send/suppress decisions.
    ...
    if (presence?.push_token && allTokens.length > 0) {
      ...
      if (idx > 0) {
        ...
        const tokenHash = presence.push_token.substring(0, 12) + '…' + presence.push_token.slice(-6);
        console.warn(`[send-driver-notification] presence.push_token not found in push_tokens (stale) — driver: ${payload.driverId}, tokenHash: ${tokenHash}. Using push_tokens table only.`);
      }
    }
