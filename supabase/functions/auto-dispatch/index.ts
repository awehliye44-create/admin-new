...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Scheduled no-preconfirmed overdue grace may raise trip.max_broadcast_rounds
    // above Admin SSOT so additional waves can use unused broadcast_round values
    // (UNIQUE trip_id,driver_id,broadcast_round). Honor the higher trip ceiling.
    const tripMaxRounds = Number(trip.max_broadcast_rounds) || 0;
    const maxRounds = Math.max(configuredMaxRounds, tripMaxRounds);
