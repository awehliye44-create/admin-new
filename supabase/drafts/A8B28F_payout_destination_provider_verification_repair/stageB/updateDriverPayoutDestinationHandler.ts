 ...
 *
 * Changes vs production v110 path:
 * - Sync UK/Revolut link failure → HTTP 422 + success:false (no false success)
 * - Persist normalized failure_class (no bank/provider secrets)
 * - Idempotent retry against existing FAILED/PENDING row (concurrency via linkage_version)
 * - NEVER writes drivers.payouts_enabled / payout_operational_paused
 * - NEVER fabricates PROVIDER_VERIFIED without counterparty+recipient refs
 *
 * NOT DEPLOYED. Copy into live path only after approval.
 */

import type { SupabaseClient } from "npm:@supabase/supabase-js@2.57.2";
import { resolveAuthenticatedDriver } from "../../functions/_shared/resolveAuthenticatedDriver.ts";
import { resolveDriverServiceAreaId } from "../../functions/_shared/resolveDriverServiceAreaId.ts";
import {
  checkServiceAreaGateway,
  gatewayNotConfiguredResponse,
} from "../../functions/_shared/paymentGatewayGuard.ts";
import {
...
import { createRevolutCounterparty } from "../../functions/_shared/revolutApi.ts";
import { ensureFreshRevolutBusinessAccessToken } from "../../functions/_shared/revolutBusinessAccessTokenRefresh.ts";
import {
...
} from "../shared/payoutDestinationVerificationOutcomeSSOT.ts";
...
async function attemptAutoRevolutLinkage(args: {
  ...
  try {
    ...
    if (updErr || !updated?.id) {
      // Concurrent retry won — do not create another counterparty; surface retry required.
      return {
        verification_status: DESTINATION_STATUS.PENDING_VERIFICATION,
        provider_link_status: PROVIDER_LINK_STATUS.FAILED,
        provider_counterparty_id: null,
