-- A8B28F Stage C BEGIN/ROLLBACK simulation (DRAFT). Must leave zero durable changes.
BEGIN;

-- Allow wallet reads in this session (MCP role is not service_role/auth.uid).
...
-- Apply Stage C function bodies (transactional; rolled back at end).
-- A8B28F Stage C — DRAFT / NOT APPLIED
...
--
-- DO NOT copy into supabase/migrations/ until explicit apply approval.
...
BEGIN
  ...
  ELSIF lower(v_driver.driver_status::text) IN ('suspended', 'banned', 'blocked', 'inactive') THEN
...
DECLARE
  ...
  v_eff boolean; v_block text; v_elig boolean;
  v_summary jsonb;
  v_ledger_n int;
  v_setting_before text;
  v_link_before text;
  v_ver_before text;
  v_op_before boolean;
...
BEGIN
  ...
  IF v_ledger_n <> 1 THEN RAISE EXCEPTION 'sim pre: ledger count %', v_ledger_n; END IF;

  SELECT setting_value::text INTO v_setting_before FROM public.admin_settings WHERE setting_key='payouts_enabled' LIMIT 1;
  SELECT provider_link_status, verification_status INTO v_link_before, v_ver_before
  ...
  -- A) MK0006 verified + not paused + legacy false ignored
  ...
  v_block := v_summary->>'early_cash_out_block_reason';
  v_elig := coalesce((v_summary->>'early_cash_out_eligible')::boolean, false);
  IF v_avail IS DISTINCT FROM 425 OR v_pend IS DISTINCT FROM 0 OR v_eff IS NOT TRUE OR v_elig IS NOT TRUE THEN
    RAISE EXCEPTION 'sim A fail avail=% pend=% eff=% elig=% block=%', v_avail, v_pend, v_eff, v_elig, v_block;
  END IF;
  RAISE NOTICE 'SIM_A_OK avail=425 pend=0 effective=true early_eligible=true';

  -- B) unverified + not paused => available cleared, canWithdraw false
  UPDATE public.driver_payout_destinations
  SET provider_link_status = 'FAILED', verification_status = 'FAILED'
  WHERE id = v_dest;
  ...
  v_summary := public.driver_wallet_summary_ssot(v_mk, NULL);
  v_block := v_summary->>'early_cash_out_block_reason';
  IF v_avail IS DISTINCT FROM 425 OR v_pend IS DISTINCT FROM 0 OR v_eff IS TRUE OR v_block IS DISTINCT FROM 'PAYOUT_ACCOUNT_NOT_VERIFIED' THEN
    RAISE EXCEPTION 'sim B fail avail=% pend=% eff=% block=%', v_avail, v_pend, v_eff, v_block;
  END IF;
  RAISE NOTICE 'SIM_B_OK unverified available=425 canWithdraw=false block=PAYOUT_ACCOUNT_NOT_VERIFIED';
  UPDATE public.driver_payout_destinations
  SET provider_link_status = v_link_before, verification_status = v_ver_before
  WHERE id = v_dest;

  -- C) verified + paused => available 0 / paused semantics
  ...
  v_summary := public.driver_wallet_summary_ssot(v_mk, NULL);
  v_block := v_summary->>'early_cash_out_block_reason';
  IF v_avail IS DISTINCT FROM 0 OR v_pend IS DISTINCT FROM 425 OR v_eff IS TRUE OR v_block IS DISTINCT FROM 'ADMIN_HOLD' THEN
    RAISE EXCEPTION 'sim C fail avail=% pend=% eff=% block=%', v_avail, v_pend, v_eff, v_block;
  END IF;
  RAISE NOTICE 'SIM_C_OK paused available=0 pending=425 block=ADMIN_HOLD';
  UPDATE public.drivers SET payout_operational_paused = v_op_before WHERE id = v_mk;

  -- D) uncleared funds => pending
  UPDATE public.payment_sessions SET captured_at = now() WHERE id = v_session;
  SELECT b.available_balance_pence, b.pending_balance_pence INTO v_avail, v_pend
  FROM public.driver_wallet_eligibility_balances(v_mk) b;
  IF v_avail IS DISTINCT FROM 0 OR v_pend IS DISTINCT FROM 425 THEN
    RAISE EXCEPTION 'sim D fail avail=% pend=%', v_avail, v_pend;
  END IF;
  RAISE NOTICE 'SIM_D_OK uncleared available=0 pending=425';
  UPDATE public.payment_sessions SET captured_at = v_captured_before WHERE id = v_session;

  -- E) global off => available cleared, withdraw blocked
  UPDATE public.admin_settings SET setting_value = 'false' WHERE setting_key = 'payouts_enabled';
  SELECT b.available_balance_pence, b.pending_balance_pence INTO v_avail, v_pend
  FROM public.driver_wallet_eligibility_balances(v_mk) b;
  v_eff := public.driver_effective_payout_allowed(v_mk);
  v_summary := public.driver_wallet_summary_ssot(v_mk, NULL);
  v_block := v_summary->>'early_cash_out_block_reason';
  IF v_avail IS DISTINCT FROM 425 OR v_pend IS DISTINCT FROM 0 OR v_eff IS TRUE OR v_block IS DISTINCT FROM 'FEATURE_DISABLED' THEN
    RAISE EXCEPTION 'sim E fail avail=% pend=% eff=% block=%', v_avail, v_pend, v_eff, v_block;
  END IF;
  RAISE NOTICE 'SIM_E_OK global_off available=425 block=FEATURE_DISABLED';
  UPDATE public.admin_settings SET setting_value = coalesce(v_setting_before, 'true') WHERE setting_key = 'payouts_enabled';

  -- F) suspended => fail closed
  UPDATE public.drivers SET driver_status = 'suspended'::public.driver_status WHERE id = v_mk;
  v_eff := public.driver_effective_payout_allowed(v_mk);
  v_summary := public.driver_wallet_summary_ssot(v_mk, NULL);
  v_block := v_summary->>'early_cash_out_block_reason';
  IF v_eff IS TRUE OR v_block IS DISTINCT FROM 'DRIVER_SUSPENDED' THEN
    RAISE EXCEPTION 'sim F fail eff=% block=%', v_eff, v_block;
  END IF;
  RAISE NOTICE 'SIM_F_OK suspended block=DRIVER_SUSPENDED';
  UPDATE public.drivers SET driver_status = v_status_before::public.driver_status WHERE id = v_mk;

  -- G) unapproved => fail closed
  UPDATE public.drivers SET approval_status = 'pending' WHERE id = v_mk;
  v_eff := public.driver_effective_payout_allowed(v_mk);
  v_summary := public.driver_wallet_summary_ssot(v_mk, NULL);
  v_block := v_summary->>'early_cash_out_block_reason';
  IF v_eff IS TRUE OR v_block IS DISTINCT FROM 'DRIVER_NOT_APPROVED' THEN
    RAISE EXCEPTION 'sim G fail eff=% block=%', v_eff, v_block;
  END IF;
  RAISE NOTICE 'SIM_G_OK unapproved block=DRIVER_NOT_APPROVED';
  UPDATE public.drivers SET approval_status = v_approval_before WHERE id = v_mk;

  -- H) DRIVER_COLLECTED exclusion still present in eligibility body
  IF pg_get_functiondef('public.driver_wallet_eligibility_balances(uuid)'::regprocedure)
       NOT LIKE '%DRIVER_COLLECTED%' THEN
    RAISE EXCEPTION 'sim H fail DRIVER_COLLECTED missing';
  END IF;
  RAISE NOTICE 'SIM_H_OK DRIVER_COLLECTED exclusion preserved';

  -- I) ledger unchanged
  IF (SELECT count(*) FROM public.driver_wallet_ledger WHERE driver_id = v_mk) IS DISTINCT FROM v_ledger_n THEN
    RAISE EXCEPTION 'sim I fail ledger mutated';
  END IF;
  IF (SELECT coalesce(sum(amount_pence),0) FROM public.driver_wallet_ledger WHERE driver_id = v_mk) IS DISTINCT FROM 425 THEN
    RAISE EXCEPTION 'sim I fail ledger sum mutated';
  END IF;
  RAISE NOTICE 'SIM_I_OK ledger unchanged';
