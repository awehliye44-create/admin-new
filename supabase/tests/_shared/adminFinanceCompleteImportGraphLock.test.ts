...
Deno.test("F3 import-graph lock — local import of computeNextWeeklyPayoutRun", async () => {
  const src = await Deno.readTextFile(
    new URL("../../functions/_shared/perDriverFinancialReconciliation.ts", import.meta.url),
  );
  assertStringIncludes(src, 'from "../../functions/_shared/payoutScheduleSSOT.ts"');
  ...
  const settlement = await Deno.readTextFile(
    new URL("../../functions/_shared/financeSettlementSummary.ts", import.meta.url),
  );
