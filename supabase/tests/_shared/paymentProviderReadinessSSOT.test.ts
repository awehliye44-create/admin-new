import { formatRevolutApiFailure, revolutHttpStatusLabel, revolutMerchantBaseUrl, validateRevolutMerchantSecret } from "../../functions/_shared/revolutApi.ts";
import { assertEquals } from "https://deno.land/std@0.168.0/testing/asserts.ts";
import {
  resolveAdapterReadinessStatus,
  resolveProviderBookingWorkflow,
} from "../../functions/_shared/paymentProviderReadinessSSOT.ts";
...
Deno.test("resolveProviderBookingWorkflow maps Revolut when production ready", () => {
  ...
  assertEquals(resolveProviderBookingWorkflow("checkout_com", true), "blocked");
});
