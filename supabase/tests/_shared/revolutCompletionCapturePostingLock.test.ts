...
} from "../../functions/_shared/postCaptureSettlementResult.ts";
...
Deno.test("revolutCompletionCapture propagates posting result and does not swallow as silent success", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/revolutCompletionCapture.ts", import.meta.url));
...
Deno.test("invokeFinalizeTripCapture retries only on HTTP/BOOT_ERROR, not wallet mismatch", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/invokeFinalizeTripCapture.ts", import.meta.url));
