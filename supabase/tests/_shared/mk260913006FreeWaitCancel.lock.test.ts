...
import { noShowEligibleFromCountedSeconds } from "../../functions/_shared/waitingSegmentClock.ts";
...
Deno.test("A2: live cancel-trip / disposition sources enforce free-wait gates", async () => {
  const cancel = await Deno.readTextFile(new URL("../../functions/cancel-trip/index.ts", import.meta.url));
  const dispose = await Deno.readTextFile(new URL("../../functions/_shared/terminalTripPaymentDisposition.ts", import.meta.url));
  const fee = await Deno.readTextFile(new URL("../../functions/_shared/terminalFeeDecisionSSOT.ts", import.meta.url));
