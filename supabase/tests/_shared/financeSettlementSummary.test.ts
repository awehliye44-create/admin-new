...
} from "../../functions/_shared/financeSettlementSummary.ts";
...
Deno.test("buildFinanceReconciliationSummary balances card ledger", async () => {
  const { computeSSOTMetrics } = await import("../../functions/_shared/financialReconciliationSSOT.ts");
...
Deno.test("FR audit: MK-260708-008 waiting £0.18 is MATCHED via PS capture breakdown", () => {
  const trip = {
    ...MK_260615_006,
    id: "7c9f1a43",
  ...
  const context = buildTripFinancialAuditContext({
    ...
    payoutItems: [],
    ledgerRows: [
      { related_trip_id: "7c9f1a43", type: "TRIP_EARNING_NET", amount_pence: 578 },
    ],
    paymentSessions: [{
      id: "ps-mk-008",
      trip_id: "7c9f1a43",
...
Deno.test("FR audit: consumes persisted PS capture_breakdown without inventing reason", () => {
  const trip = {
    ...
    trip_code: "MK-260708-008",
    final_customer_fare_pence: 680,
    final_fare_pence: 680, // stale fare without waiting — PS metadata is owner
    commission_pence: 102,
    driver_net_pence: 578,
  };
  const context = buildTripFinancialAuditContext({
