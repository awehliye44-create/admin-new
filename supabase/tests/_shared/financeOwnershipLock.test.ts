...
import { FR_TRIP_AUDIT_STATUS } from "../../functions/_shared/frConsumeOnlySSOT.ts";

Deno.test("FR monitor does not insert or retry Driver Wallet Ledger money", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/financial-ssot-monitor/index.ts", import.meta.url));
...
Deno.test("FR contains no captured-trip wallet recovery / money write path", async () => {
  const files = [
    "../../functions/financial-ssot-monitor/index.ts",
    "../../functions/_shared/frPerTripAuditSSOT.ts",
    "../../functions/_shared/frConsumeOnlySSOT.ts",
    "../../functions/_shared/frTripAuditComparisonSSOT.ts",
    "../../functions/admin-finance-reconciliation/index.ts",
  ];
