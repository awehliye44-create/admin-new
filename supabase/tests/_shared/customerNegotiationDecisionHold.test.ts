...
} from "../../functions/_shared/customerNegotiationDecisionHold.ts";
...
Deno.test("expire-offers and sync skip in-flight responded_at holds", async () => {
  const expire = await Deno.readTextFile(new URL("../../functions/expire-offers/index.ts", import.meta.url));
