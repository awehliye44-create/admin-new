...
Deno.test("stop-workflow imports getDriverCommissionPct, not the removed getDriverCommission", async () => {
  ...
  const commission = await Deno.readTextFile(
    new URL("../../functions/_shared/commission.ts", import.meta.url),
  );
  assertEquals(src.includes('from "../../functions/_shared/commission.ts"'), true);
...
Deno.test("stop-workflow statically imports capture helpers used on complete_trip", async () => {
  ...
  const capture = await Deno.readTextFile(
    new URL("../../functions/_shared/digitalPaymentCapture.ts", import.meta.url),
  );
  const provider = await Deno.readTextFile(
    new URL("../../functions/_shared/tripPaymentProviderSSOT.ts", import.meta.url),
  );
  assertEquals(src.includes('from "../../functions/_shared/digitalPaymentCapture.ts"'), true);
  assertEquals(src.includes('from "../../functions/_shared/tripPaymentProviderSSOT.ts"'), true);
  ...
  assertEquals(src.includes('await import("../../functions/_shared/digitalPaymentCapture.ts")'), false);
  assertEquals(src.includes('await import("../../functions/_shared/tripPaymentProviderSSOT.ts")'), false);
...
Deno.test("post-capture settlement persists stamp columns with wallet credit", async () => {
  const src = await Deno.readTextFile(
    new URL("../../functions/_shared/applyCanonicalSettlementAfterCapture.ts", import.meta.url),
  );
...
Deno.test("complete_trip card path delegates wallet posting to finalize-trip-and-capture", async () => {
  ...
  const settlement = await Deno.readTextFile(
    new URL("../../functions/_shared/tripSettlement.ts", import.meta.url),
  );
  const invoke = await Deno.readTextFile(
    new URL("../../functions/_shared/invokeFinalizeTripCapture.ts", import.meta.url),
  );
  assertEquals(src.includes("tripSettlementDbColumns"), true);
  assertEquals(src.includes('from "../../functions/_shared/tripSettlement.ts"'), true);
  ...
  assertEquals(src.includes('from "../../functions/_shared/applyCanonicalSettlementAfterCapture.ts"'), false);
  assertEquals(src.includes('from "../../functions/_shared/capturedTripWalletRecovery.ts"'), false);
