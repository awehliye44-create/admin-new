...
Deno.test("payoutLedgerSync has no direct wallet debit insert", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/payoutLedgerSync.ts", import.meta.url));
...
Deno.test("payout completion RPC is sole finalize_driver_payout_completion edge wrapper", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/payoutCompletionRpcSSOT.ts", import.meta.url));
...
Deno.test("legacy trip earning paths delegate to canonicalTypedWalletPostingSSOT", async () => {
  const paths = [
    "../../functions/admin-payment-detail/index.ts",
    "../../functions/record-financial-outcome/index.ts",
    "../../functions/repair-commissions/index.ts",
    "../../functions/stop-workflow/index.ts",
    "../../functions/_shared/noShowSettlement.ts",
  ];
...
Deno.test("tripSettlement delegates standard path to calculateCanonicalSettlement", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/tripSettlement.ts", import.meta.url));
...
Deno.test("canonicalTypedWalletPostingSSOT owns TRIP_EARNING_NET posting entry", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/canonicalTypedWalletPostingSSOT.ts", import.meta.url));
...
Deno.test("financeOwnershipLock tests still pass paths unchanged", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/financeOwnershipLock.test.ts", import.meta.url));
  assertStringIncludes(src, "creditCapturedCardTripLedger");
});
