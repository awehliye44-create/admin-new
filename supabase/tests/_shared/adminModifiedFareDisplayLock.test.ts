...
import { computeLiveTripFarePreview } from "../../functions/_shared/liveTripFareSSOT.ts";
...
Deno.test("LOCK: frontend and edge liveTripFareSSOT stay in sync", async () => {
  ...
  const edge = await Deno.readTextFile(new URL("../../functions/_shared/liveTripFareSSOT.ts", import.meta.url));
  assertEquals(frontend, edge);
});

Deno.test("LOCK: committed fare defaults audit-only when metadata missing", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/liveTripFareSSOT.ts", import.meta.url));
