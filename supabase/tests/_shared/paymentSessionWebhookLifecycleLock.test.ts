...
} from "../../functions/_shared/paymentSessionWebhookLifecycleResolver.ts";
import { applyPaymentSessionWebhookLifecycleUpdate } from "../../functions/_shared/applyPaymentSessionWebhookLifecycleUpdate.ts";
...
Deno.test("webhook resolver module does not import wallet/settlement/provider-money", async () => {
  const resolverSrc = await Deno.readTextFile(
    new URL("../../functions/_shared/paymentSessionWebhookLifecycleResolver.ts", import.meta.url),
  );
  const applySrc = await Deno.readTextFile(
    new URL("../../functions/_shared/applyPaymentSessionWebhookLifecycleUpdate.ts", import.meta.url),
  );
...
Deno.test("admin refresh imports monotonic lifecycle apply helper", async () => {
  const refreshSrc = await Deno.readTextFile(
    new URL("../../functions/admin-refresh-payment-sessions/index.ts", import.meta.url),
  );
  assertEquals(
