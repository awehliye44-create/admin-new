...
import { isInstantInClosedRange } from "../../functions/_shared/economicEarnedAtSSOT.ts";

Deno.test("admin-driver-invoice handler actions: preview is sample/stored HTML; generate/send are writes", async () => {
  const src = await Deno.readTextFile(
    new URL("../../functions/admin-driver-invoice/index.ts", import.meta.url),
  );
...
Deno.test("invoice aggregation consumes SQL economic clocks; TEN never falls back to created_at", async () => {
  const src = await Deno.readTextFile(
    new URL("../../functions/_shared/driverInvoiceAggregation.ts", import.meta.url),
  );
