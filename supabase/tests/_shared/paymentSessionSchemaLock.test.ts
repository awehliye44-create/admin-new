...
} from "../../functions/_shared/paymentSessionCaptureGateSSOT.ts";
import {
  tripWalletRecoverySelectColumns,
} from "../../functions/_shared/capturedTripWalletRecovery.ts";
...
Deno.test("wallet posting path reads trip financial_model from trips, not payment_sessions", async () => {
  const applySrc = await readSharedSource("../../functions/_shared/applyCanonicalSettlementAfterCapture.ts");
  ...
  const gateSrc = await readSharedSource("../../functions/_shared/paymentSessionCaptureGateSSOT.ts");
...
Deno.test("capturedTripWalletRecovery uses shared recovery session loader without payment_sessions financial_model", async () => {
  const recoverySrc = await readSharedSource("../../functions/_shared/capturedTripWalletRecovery.ts");
...
Deno.test("lifecycle finalizer identity lookup selects id only from payment_sessions", async () => {
  const src = await readSharedSource("../../functions/_shared/paymentSessionLifecycleFinalizer.ts");
