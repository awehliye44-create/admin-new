...
Deno.test("createCallLog stamps expires_at for timeout sweep", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/callMaskingLogs.ts", import.meta.url));
...
Deno.test("createOrReuseVoipSession supersedes active call_masking", async () => {
  const src = await Deno.readTextFile(new URL("../../functions/_shared/voipCallLogs.ts", import.meta.url));
...
Deno.test("timeout sweep terminates null-expires_at masking orphans", async () => {
  const src = await Deno.readTextFile(
    new URL("../../functions/trip-communication-timeout-sweep/index.ts", import.meta.url),
  );
