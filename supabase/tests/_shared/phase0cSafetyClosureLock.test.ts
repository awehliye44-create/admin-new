...
Deno.test("payout TS wrapper does not replace SQL invariant enforcement", async () => {
  const rpc = await Deno.readTextFile(new URL("../../functions/_shared/payoutCompletionRpcSSOT.ts", import.meta.url));
...
Deno.test("DIRECT_RPC_BYPASS_SAFE — automated callers may invoke SQL directly", async () => {
  const callers = [
    "../../functions/driver-withdraw/index.ts",
    "../../functions/admin-finalize-driver-payout-completion/index.ts",
    "../../functions/_shared/driverWithdrawProviderReconcile.ts",
  ];
