...
import { buildCustomerNegotiationView } from "../../functions/_shared/customerNegotiationView.ts";
import {
  negotiationDeadlineIso,
  resolveNegotiationCountdownSeconds,
  resolveNegotiationDeadlineIso,
} from "../../functions/_shared/negotiation-deadline.ts";
import {
...
} from "../../functions/_shared/negotiationPushCopy.ts";
import {
  buildNegotiationFromPresetOptions,
  PRESET_SLOT_COUNT,
} from "../../functions/_shared/presetOptionsCanonical.ts";
import {
...
} from "../../functions/_shared/presetNegotiationEligibility.ts";
...
Deno.test("customer fare push resolves customers.id to auth user id", async () => {
  const auth = await Deno.readTextFile(
    new URL("../../functions/_shared/authoritativeDevicePush.ts", import.meta.url),
  );
  const offer = await Deno.readTextFile(
    new URL("../../functions/driver-fare-offer/index.ts", import.meta.url),
  );
  const tripNotif = await Deno.readTextFile(
    new URL("../../functions/send-trip-notification/index.ts", import.meta.url),
  );
...
Deno.test("accept-offer blocks other drivers while pre-held", async () => {
  const src = await Deno.readTextFile(
    new URL("../../functions/accept-offer/index.ts", import.meta.url),
  );
...
Deno.test("resume search does not reopen consumed negotiation", async () => {
  const src = await Deno.readTextFile(
    new URL("../../functions/customer-resume-driver-search/index.ts", import.meta.url),
  );
...
Deno.test("Driver→Customer and Customer→Driver stamp Admin countdown, not 20s", async () => {
  ...
  const decision = await Deno.readTextFile(
    new URL("../../functions/customer-fare-decision/index.ts", import.meta.url),
  );
  const deadline = await Deno.readTextFile(
    new URL("../../functions/_shared/negotiation-deadline.ts", import.meta.url),
  );
...
Deno.test("expiry rematches and never auto-accepts", async () => {
  const expire = await Deno.readTextFile(
    new URL("../../functions/expire-offers/index.ts", import.meta.url),
  );
  const sync = await Deno.readTextFile(
    new URL("../../functions/customer-negotiation-sync/index.ts", import.meta.url),
  );
...
Deno.test("£Y timeout and Decline enter the same Driver second chance; £Z stays committed", async () => {
  ...
  const final = await Deno.readTextFile(
    new URL("../../functions/driver-fare-final/index.ts", import.meta.url),
  );
  const grace = await Deno.readTextFile(
    new URL("../../functions/_shared/customerNegotiationGrace.ts", import.meta.url),
  );
...
Deno.test("original Accept and negotiated Accept still assign immediately", async () => {
  ...
  const driverPush = await Deno.readTextFile(
    new URL("../../functions/send-driver-notification/index.ts", import.meta.url),
  );
...
Deno.test("SQL enrichment skips stacked and accept_stacked_ride enforces hold", async () => {
  ...
  const lifecycle = await Deno.readTextFile(
    new URL("../../functions/_shared/stackedRideLifecycle.ts", import.meta.url),
  );
