...
} from "../../functions/admin-payment-sessions/handler.ts";
import type { GateError, GateResult } from "../../functions/_shared/adminPaymentGate.ts";
...
Deno.test("source lock: no financeSafeJson always-200 coercer", async () => {
  const indexSrc = await Deno.readTextFile(
    new URL("../../functions/admin-payment-sessions/index.ts", import.meta.url),
  );
  const handlerSrc = await Deno.readTextFile(
    new URL("../../functions/admin-payment-sessions/handler.ts", import.meta.url),
  );
