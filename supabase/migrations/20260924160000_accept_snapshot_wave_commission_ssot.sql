-- Accept-time fare_snapshot_json must not keep stale v1 global_base commission
-- alongside the accepted wave snapshot (MK-260815-010: 74p/421p + 20p/475p).
-- Reuse existing keys. Do not rewrite historical trips.
...
BEGIN
  ...
  SET accepted_dispatch_wave = v_offer.dispatch_wave,
      ...
      fare_snapshot_json = COALESCE(fare_snapshot_json, '{}'::jsonb) || jsonb_build_object(
        ...
        'platform_net_revenue_pence', v_commission,
...
BEGIN
  ...
  IF v_settle THEN
    ...
    IF v_use_wave THEN
      ...
      v_commissionable_pence := GREATEST(0, p_committed_fare_pence - v_airport_pence);
    ELSE
  ...
  SET
    ...
    fare_snapshot_json = COALESCE(fare_snapshot_json, '{}'::jsonb)
      || jsonb_strip_nulls(jsonb_build_object(
        ...
        'platform_net_revenue_pence', CASE WHEN v_settle THEN v_commission_pence ELSE NULL END,
