-- Rollback: restore pre-allowlist reserve_driver_payout_item (WEEKLY_SCHEDULED-only kind gate).
-- FAIL CLOSED if any EARLY_CASHOUT reservation/payment is in flight.
...
BEGIN
  ...
  WHERE pb.kind = 'EARLY_CASHOUT'
    AND (
      upper(coalesce(pi.status, '')) IN (
        'RESERVED', 'RESERVING', 'SUBMITTING', 'SUBMITTED', 'PROCESSING', 'SENT'
      )
      OR EXISTS (
        SELECT 1 FROM public.driver_payout_reservations r
        WHERE r.payout_item_id = pi.id AND upper(coalesce(r.status, '')) = 'ACTIVE'
      )
      OR EXISTS (
        SELECT 1 FROM public.driver_payout_payment_intents i
        WHERE i.payout_item_id = pi.id
      )
    );
  IF v_inflight > 0 THEN
    RAISE EXCEPTION 'ROLLBACK_BLOCKED_EARLY_CASHOUT_IN_FLIGHT count=%', v_inflight;
  END IF;
END $$;
