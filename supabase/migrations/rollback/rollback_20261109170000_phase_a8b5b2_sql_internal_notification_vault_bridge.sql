-- ============================================================
-- Rollback Phase A8B5B2-SQL (credential-safe, Vault bridge retained)
-- NOT for ordinary Edge incidents — see Level A vs Level B below.
--
-- Level A — Edge rollback (preferred when gate breaks delivery):
--   1) Redeploy send-driver-notification v513
--   2) Leave this SQL Vault bridge installed
--   3) Retain Vault secret onecab_internal_notification_token
--   4) Retain Edge secret ONECAB_INTERNAL_NOTIFICATION_TOKEN
--   → restores public endpoint availability; SQL notifications keep working
--     (v513 ignores the internal header; future strict Edge requires it)
--   → DO NOT run this SQL rollback file for Level A
--
-- Level B — Full database rollback (only if business/payload bodies must revert):
--   - Re-assert Vault-header caller bodies (same auth bridge as forward)
--   - KEEP public.onecab_internal_notification_http_headers()
--   - NEVER re-embed anon JWT
--   - NEVER call cron_edge for SDN authentication
--   - NEVER depend on empty service-role/anon GUCs
--   - NEVER DROP the Vault helper while SQL callers depend on it
--   - Secret deletion is LAST and only after no caller uses it
--
-- Forward A8B5B2 only changed authentication headers, so Level B bodies match
-- the forward Vault-header drafts (business/payload/error paths unchanged).
-- Does not invoke notification functions or mutate trips/offers.
-- ============================================================

BEGIN;

-- Keep / re-assert postgres-only Vault header helper (do not drop).
...
-- Level B safe body (Vault header): notify_driver_lost_property md5=b52d209c5895922861ccccc4e457dfc5
...
-- Level B safe body (Vault header): notify_driver_on_trip_cancelled md5=8ba90beb3e1ec481429da11b1b6f4dfc
...
-- Level B safe body (Vault header): notify_driver_trip_change_request md5=5e8568cfe63e711319b5ed3ae2922add
...
-- Level B safe body (Vault header): notify_drivers_trip_cancelled md5=ca0d8b71d751e29a9a0259c3dc678180
...
-- Level B safe body (Vault header): notify_offer_drivers_on_trip_terminal md5=f4ccda04dd2893f5e5c0a68b8a8791f0
...
-- Level B safe body (Vault header): ride_offer_dispatch_push_delivery md5=659c1732319068fe4ffe92e0714c3bc4
...
-- Level B safe body (Vault header): tg_driver_alerts_push_on_raise md5=0de55487c56452d3d715fcf549009279
...
-- Intentionally NO DROP of onecab_internal_notification_http_headers().
