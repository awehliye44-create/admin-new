-- ============================================================
-- ROLLBACK for 20260930150000_provider_refund_ledger_idempotency.sql
--
-- Never deletes money rows. Does NOT edit schema_migrations.
-- Refuses to restore broad unique_trip_ledger_entry when incompatible.
-- ============================================================
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'ROLLBACK REFUSED: non-NULL provider_refund_id REFUND_DEBIT rows exist';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'ROLLBACK REFUSED: multiple REFUND_DEBIT rows per trip — cannot restore unique_trip_ledger_entry';
  END IF;
END $$;
...
-- payment_session_refunds_provider_refund_unique may pre-exist migration; do not drop if present before migration.

ALTER TABLE public.driver_wallet_ledger
  DROP COLUMN IF EXISTS payment_provider,
  DROP COLUMN IF EXISTS provider_refund_id;

-- Restore broad uniqueness only when no duplicate (related_trip_id,type) pairs exist.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
  ...
  ELSE
    RAISE NOTICE 'ROLLBACK: skipped unique_trip_ledger_entry restore — duplicate rows present';
  END IF;
END $$;
