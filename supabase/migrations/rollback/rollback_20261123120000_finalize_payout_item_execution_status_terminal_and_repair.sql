-- Rollback: restore exact pre-20261123120000 finalize body and revert repaired rows.
-- Fail closed if the two MK0006 items are no longer in repaired COMPLETED/COMPLETED state.
...
BEGIN
  UPDATE public.payout_items
  SET execution_status = 'SUBMITTED', updated_at = now()
  WHERE id IN (
    '5f00ba74-9592-4438-9e42-a9ae9a368c6a'::uuid,
    '0e237504-4d22-4ce4-a93d-ee2c2bdce004'::uuid
  )
    AND upper(status) = 'COMPLETED'
    AND upper(coalesce(execution_status, '')) = 'COMPLETED';

  GET DIAGNOSTICS v_n = ROW_COUNT;
  IF v_n IS DISTINCT FROM 2 THEN
    RAISE EXCEPTION 'rollback failed: expected 2 repaired rows, updated %', v_n;
  END IF;
END $$;

DROP FUNCTION IF EXISTS public.repair_stale_completed_payout_item_execution_status(uuid);
