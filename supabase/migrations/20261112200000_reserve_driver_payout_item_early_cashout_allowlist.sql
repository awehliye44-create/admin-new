-- Forward: restore EARLY_CASHOUT on reserve_driver_payout_item kind allow-list.
-- Live Stage C / sibling rewrite allowed WEEKLY_SCHEDULED only (WEEKLY_MONDAY dead path).
-- Driver Withdraw Edge creates EARLY_CASHOUT batches → BATCH_NOT_ELIGIBLE before reservation.
-- Preserves live body (PROCESSING sibling, Stage C effective payout helper, lineage assert).
-- Grants: postgres + service_role EXECUTE; SECURITY DEFINER; search_path=public.
-- PREPARED NOT APPLIED — requires explicit approval after containment gates.
...
BEGIN
  ...
  -- Canonical allow-list only: WEEKLY_SCHEDULED + EARLY_CASHOUT.
  -- WEEKLY_MONDAY remains non-reservable (legacy read-only).
  -- Restores Driver Withdraw after Stage C / sibling rewrites dropped EARLY_CASHOUT.
...
GRANT EXECUTE ON FUNCTION public.reserve_driver_payout_item(uuid) TO postgres;
