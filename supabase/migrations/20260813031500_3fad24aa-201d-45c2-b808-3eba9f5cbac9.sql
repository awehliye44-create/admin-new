-- 1. Settings: per-wave commission incentive (Auto-Dispatch Rules SSOT)
...
-- 2. Offer-level economics snapshot
...
-- 3. Trip-level acceptance snapshot + monotonic incentive floor
...
-- 4. Commission resolution: tier dependency removed (dispatch settings SSOT)
...
DECLARE
  v_pct numeric;
BEGIN
  PERFORM p_driver_id, p_service_area_id;
  SELECT base_driver_commission_percent INTO v_pct
  FROM public.global_dispatch_settings WHERE singleton = true LIMIT 1;
  RETURN LEAST(100, GREATEST(0, COALESCE(v_pct, 0)))::numeric;
END;
$function$;

-- Wave incentive resolver: base - wave reduction (never negative, never below floor)
CREATE OR REPLACE FUNCTION public.resolve_wave_commission_percent(
  p_wave integer,
  p_floor_reduction_percent numeric DEFAULT 0
)
RETURNS TABLE(base_percent numeric, reduction_percent numeric, effective_percent numeric)
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  v_base := LEAST(100, GREATEST(0, COALESCE(v_g.base_driver_commission_percent, 0)));
  v_red := CASE p_wave
    WHEN 1 THEN COALESCE(v_g.wave1_commission_reduction_percent, 0)
    WHEN 2 THEN COALESCE(v_g.wave2_commission_reduction_percent, 0)
    ELSE COALESCE(v_g.wave3_commission_reduction_percent, 0)
  END;

  -- Monotonic: incentive can only improve across waves/rounds.
  v_red := GREATEST(COALESCE(v_red, 0), COALESCE(p_floor_reduction_percent, 0));
  v_red := LEAST(v_base, GREATEST(0, v_red));

  base_percent := v_base;
  reduction_percent := v_red;
  effective_percent := GREATEST(0, v_base - v_red);
  RETURN NEXT;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.resolve_wave_commission_percent(integer, numeric) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.resolve_wave_commission_percent(integer, numeric) TO authenticated, service_role;

-- 5. Dispatch engine: repeating 3-wave rounds + per-wave incentive snapshot
...
BEGIN
  ...
  v_max_rounds       := GREATEST(1, COALESCE(v_g.max_dispatch_rounds, 3));
  ...
  WITH base AS (
    ...
    WHERE d.approval_status = 'approved' AND d.documents_approved = true
      ...
      -- Hard exclusions: declined / accepted / live offers are never re-offered.
      AND NOT EXISTS (
        SELECT 1 FROM public.ride_offers ro
...
-- 6. Lifecycle: wave sequence limit = 3 waves x max rounds, TTL is the hard stop
...
BEGIN
  ...
  SELECT GREATEST(1, COALESCE(max_dispatch_rounds, 3)) INTO v_max_rounds
  FROM public.global_dispatch_settings WHERE singleton = true LIMIT 1;

  v_seq := COALESCE(v_trip.current_broadcast_round, 0);
  v_max_seq := COALESCE(v_max_rounds, 3) * 3;
...
-- 7. Acceptance: snapshot the accepted wave economics onto the trip (settlement SSOT)
