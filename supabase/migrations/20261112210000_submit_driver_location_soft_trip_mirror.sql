-- Phase: soft-fail stale p_trip_id on submit_driver_location_sample
--
-- Product: driver_presence is ONLINE fleet SSOT and must advance even when the
-- client sends a stale/invalid active trip id (BG latch / remount races).
-- trip_driver_live_location is Customer trip SSOT and is mirrored only when
-- p_trip_id still belongs to this driver on a live trackable trip.
--
-- Before: TRIP_ASSIGNMENT_REJECTED aborted the entire RPC → presence froze while
-- Maps was foreground and the latch held a terminal/stale trip id.
-- After: presence upsert always proceeds (subject to existing sequence/GPS
-- guards); invalid trip ids skip the Customer mirror only.
...
BEGIN
  ...
  -- Soft demote: active_trip* without trip id must not fail fleet presence.
  ...
  -- Soft-validate trip association. Never abort presence for a stale trip id.
