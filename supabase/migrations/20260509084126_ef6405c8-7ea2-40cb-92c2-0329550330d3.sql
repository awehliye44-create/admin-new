...
BEGIN
  ...
  -- Compute deduped metrics AND raw duplicate diagnostics in one CTE block
  WITH bdl_raw AS (
  ...
  SELECT
    d.offered, d.received, d.push_enqueued, d.push_sent,
    GREATEST(r.raw_ack - d.received, 0),
    GREATEST(r.raw_push_sent - d.push_sent, 0),
    r.retry_delivery,
    r.pending_recovery
  INTO
    v_offered, v_received, v_push_enqueued, v_push_sent,
  ...
  -- Timeline: dedupe by key within bucket
