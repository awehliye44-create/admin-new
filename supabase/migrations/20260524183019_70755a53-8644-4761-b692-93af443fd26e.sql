-- =====================================================================
-- 1. stop_waiting_settings (per service area) — fare lifecycle config
--    Moved out of dispatch_settings (which is being deprecated).
-- =====================================================================
CREATE TABLE IF NOT EXISTS public.stop_waiting_settings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  service_area_id uuid NOT NULL UNIQUE REFERENCES public.service_areas(id) ON DELETE CASCADE,
  stop_radius_enabled boolean NOT NULL DEFAULT true,
  stop_radius_meters integer NOT NULL DEFAULT 100,
  stop_waiting_grace_period_seconds integer NOT NULL DEFAULT 180,
  stop_waiting_charge_interval_seconds integer NOT NULL DEFAULT 10,
  stop_waiting_rate_pence_per_minute integer NOT NULL DEFAULT 30,
  stop_waiting_max_minutes integer,
...
INSERT INTO public.stop_waiting_settings (
  ...
  stop_waiting_grace_period_seconds, stop_waiting_charge_interval_seconds,
  stop_waiting_rate_pence_per_minute, stop_waiting_max_minutes
)
SELECT
  d.service_area_id,
  COALESCE(d.stop_radius_enabled, true),
  COALESCE(d.stop_radius_meters, 100),
  COALESCE(d.stop_waiting_grace_period_seconds, 180),
  COALESCE(d.stop_waiting_charge_interval_seconds, 10),
  COALESCE(d.stop_waiting_rate_pence_per_minute, 30),
  d.stop_waiting_max_minutes
...
DECLARE
  ...
  -- Helper-style return builder via local fn not supported; we use a single template.
BEGIN
