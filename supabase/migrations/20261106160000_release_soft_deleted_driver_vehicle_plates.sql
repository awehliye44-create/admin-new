-- Soft-deleted drivers must not keep exclusive claim on license plates.
-- Admin delete anonymises email/phone but left vehicles attached → re-signup
-- with the same plate raised VEHICLE_OWNERSHIP_CONFLICT, which the Driver app
-- then mis-mapped as "email or phone already connected".

BEGIN;

-- 1) Release plates held by soft-deleted drivers (bypass vehicle edit locks).
SET LOCAL session_replication_role = replica;
UPDATE public.vehicles v
SET license_plate = 'DELETED-' || v.id::text
FROM public.drivers d
WHERE v.driver_id = d.id
  AND d.deleted_at IS NOT NULL
  AND v.license_plate IS NOT NULL
  AND v.license_plate NOT LIKE 'DELETED-%';
SET LOCAL session_replication_role = DEFAULT;

-- 2) On future soft-delete, scrub plates (bypass user triggers the same way).
CREATE OR REPLACE FUNCTION public.drivers_release_vehicles_on_soft_delete()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  THEN
    ...
    SET license_plate = 'DELETED-' || id::text
    WHERE driver_id = NEW.id
      AND license_plate IS NOT NULL
      AND license_plate NOT LIKE 'DELETED-%';
...
DROP TRIGGER IF EXISTS trg_drivers_release_vehicles_on_soft_delete ON public.drivers;
CREATE TRIGGER trg_drivers_release_vehicles_on_soft_delete
  AFTER UPDATE OF deleted_at ON public.drivers
  FOR EACH ROW
  EXECUTE FUNCTION public.drivers_release_vehicles_on_soft_delete();

COMMENT ON FUNCTION public.drivers_release_vehicles_on_soft_delete() IS
  'When a driver is soft-deleted, scrub vehicle plates so the registration can be reused.';

-- 3) Finalize ownership check: only active (non-deleted) drivers block a plate.
