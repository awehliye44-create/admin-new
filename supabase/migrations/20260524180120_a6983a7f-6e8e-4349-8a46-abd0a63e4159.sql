-- ============================================================
-- P0 FIX 1: Repoint expire-offers-sweep cron to call SQL directly
-- ============================================================
DO $$
BEGIN
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'trip_id', p_trip_id, 'status', 'trip_not_found',
  ...
  -- Idempotent round-advance guard: tightened to (trip_id, previous_round)
  BEGIN
  ...
  IF v_trip.negotiation_owner_driver_id IS NOT NULL OR v_trip.status = 'negotiating' THEN
    RETURN jsonb_build_object('trip_id',p_trip_id,'trip_code',v_trip.trip_code,'round',v_prev_round,
      'status','skipped','offers_created',0,'offer_ids','[]'::jsonb,
      'selected_driver_ids','[]'::jsonb,'skipped_driver_ids','[]'::jsonb,
  ...
  IF v_round > v_max_rounds THEN
    ...
    RETURN jsonb_build_object('trip_id',p_trip_id,'trip_code',v_trip.trip_code,'round',v_round,
      'status','exhausted','offers_created',0,'offer_ids','[]'::jsonb,
      'selected_driver_ids','[]'::jsonb,'skipped_driver_ids','[]'::jsonb,
  ...
  -- Capture skipped driver IDs (hard excluded)
