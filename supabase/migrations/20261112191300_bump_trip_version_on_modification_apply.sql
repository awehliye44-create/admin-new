-- Bump trip_version on every modification apply so Driver does not reject
-- subsequent no-payment-increase changes as stale_version (MK-260916-041).
-- fare_revision_number bumps when the payable changes.
-- Rollback: rollback/rollback_20261112191300_bump_trip_version_on_modification_apply.sql
BEGIN;

CREATE OR REPLACE FUNCTION public.apply_trip_modification_to_trip(p_trip_id uuid, p_change_type text, p_fare_delta_pence integer, p_new_fare_pence integer, p_new_distance_meters integer, p_new_duration_seconds integer, p_before_snapshot jsonb, p_after_snapshot jsonb, p_fare_preview jsonb DEFAULT NULL::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_trip.fare_locked OR v_locked_base IS NOT NULL THEN
    UPDATE trips SET
      ...
      driver_net_before_tip_pence = v_driver_net_pence,
      ...
      END,
      trip_version = COALESCE(trip_version, 0) + 1,
      fare_revision_number = CASE
        WHEN COALESCE(p_fare_delta_pence, 0) <> 0
          OR COALESCE(p_new_fare_pence, 0) IS DISTINCT FROM COALESCE(final_customer_fare_pence, 0)
        THEN COALESCE(fare_revision_number, 0) + 1
        ELSE fare_revision_number
      END,
      updated_at = now()
    WHERE id = p_trip_id;
  ELSE
