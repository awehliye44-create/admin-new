-- MK-260817-006: SQL pending must match JS isScheduledInstantConversionPending.
-- Thin rows may omit dispatch_mode but still have is_scheduled / scheduled_at.
-- Isolated: helper + expire RPC + TTL trigger. Does not mutate MK-260817-006.

CREATE OR REPLACE FUNCTION public.is_scheduled_instant_conversion_pending(
...
AS $function$
  SELECT
    lower(replace(trim(COALESCE(p_scheduled_status, '')), '-', '_'))
      IS DISTINCT FROM 'converted_to_instant'
    AND lower(replace(trim(COALESCE(p_dispatch_mode, '')), '-', '_'))
      IS DISTINCT FROM 'instant'
    AND (
      lower(replace(trim(COALESCE(p_dispatch_mode, '')), '-', '_')) = 'scheduled'
      OR COALESCE(p_is_scheduled, false) = true
      OR p_scheduled_at IS NOT NULL
    );
$function$;

COMMENT ON FUNCTION public.is_scheduled_instant_conversion_pending(text, text, boolean, timestamptz) IS
  'MK-260817-006: true while scheduled→instant conversion has not run. Matches Edge isScheduledInstantConversionPending.';
...
COMMENT ON FUNCTION public.expire_trip_when_search_exhausted(uuid) IS
  'Expire searching trips after searching_expires_at. Scheduled handover ignores stale searching_expires_at and never uses created_at instant TTL.';
...
COMMENT ON FUNCTION public.trg_scheduled_handover_block_premature_search_ttl() IS
  'MK-260817-006: while scheduled conversion is pending, do not stamp instant TTL or system-expire the trip.';
