-- MK-260904-003: location_frozen must not hard-exclude intentional online drivers
-- on trip_insert (dispatch_trip_offers text overload). Align with JS auto-dispatch
-- evaluateDispatchableReadiness: stale/frozen GPS is degradable when push+intent.
-- Do not apply this migration to production until ops approve.
-- READ-ONLY audit first; this ships the code fix for review/deploy.
