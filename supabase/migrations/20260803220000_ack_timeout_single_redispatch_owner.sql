-- Ack-timeout chain hardening (instant dispatch only).
-- 1) Expire ONLY pending, unacked offers whose authoritative expires_at has passed.
-- 2) Never invent a shorter window than Admin wave expiry (no hard 20s steal).
-- 3) Clear trip offer pointer only when it still points at this offer's driver.
-- 4) Do NOT HTTP-invoke auto-dispatch here — ack-timeout-sweep Edge owns
--    redispatch once (prevents double invoke / BOOT_ERROR races).
-- 5) Record GDS config snapshot + round for observability.
...
BEGIN
  ...
  -- If Admin SSOT is missing columns, keep NULL — do not invent Admin UI numbers.
  ...
  FOR r IN
    ...
    WHERE ro.status = 'pending'
      ...
      AND (
        -- Authoritative: only after backend expires_at.
        (ro.expires_at IS NOT NULL AND ro.expires_at <= v_now)
        OR (
          -- Legacy rows missing expires_at: derive from GDS wave for that round.
  ...
  LOOP
    ...
    WHERE o.id = r.oid
      ...
      -- Never expire an offer that was accepted/declined concurrently.
    ...
    RAISE LOG '[delivery] ack_timeout_sweep booking_id=% offer_id=% driver_id=% round=% expires_at=% timeout_at=% phase=offer_expired gds_id=%',
    ...
    -- Only clear the live pointer when it still names this timed-out driver.
    ...
    -- Redispatch is owned exclusively by Edge ack-timeout-sweep (single flight).
...
COMMENT ON FUNCTION public.process_ride_offer_ack_timeouts() IS
  'Expire pending unacked ride_offers only after expires_at (Admin wave SSOT). Redispatch owned by ack-timeout-sweep Edge.';
