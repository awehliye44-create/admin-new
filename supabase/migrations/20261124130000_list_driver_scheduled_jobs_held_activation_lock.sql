-- Admin HELD + activation NRO list hardening for Driver Scheduled Jobs.
-- 1) Confirmed includes awaiting_activation_accept (commitment NRO pending Accept).
-- 2) Never emit Start journey as primary_action / CTA (activation is NRO Accept only).
-- 3) Requested marketplace excludes admin_held and null/unreached broadcast_at.
      ...
      (
        ...
        FROM (
          SELECT
            ...
            CASE
              WHEN t.scheduled_status = 'awaiting_activation_accept'
                THEN 'confirmed'
          ...
          WHERE t.dispatch_mode = 'scheduled'
            ...
            AND t.scheduled_status IN ('driver_assigned', 'awaiting_activation_accept')
