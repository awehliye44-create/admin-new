-- booking_received_miss must not steal the Admin wave offer window.
-- Wait until the offer's expires_at (set from wave1/2/3_offer_expiry_seconds) before expiring unacked offers.
...
DECLARE
  ...
  v_wave1_secs int := 45;
BEGIN
  SELECT COALESCE(wave1_offer_expiry_seconds, 45)
    INTO v_wave1_secs
  ...
  FOR r IN
    ...
    WHERE ro.status = 'pending'
      ...
      -- Respect Admin per-wave expiry on the offer row; fall back to wave1 SSOT.
      AND COALESCE(
            ro.expires_at,
            COALESCE(ro.delivery_first_dispatched_at, ro.offered_at)
              + make_interval(secs => GREATEST(v_wave1_secs, 20))
          ) <= v_now
  ...
  LOOP
    ...
    PERFORM public.record_booking_delivery(
      ...
      v_offer_id,
      'postgres',
      jsonb_build_object('timeout_at', v_now, 'policy', 'offer_expires_at')
    );
