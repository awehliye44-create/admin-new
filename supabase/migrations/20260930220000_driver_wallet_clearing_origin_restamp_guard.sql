-- P0: Stop false 27h pending from admin-restamped captured_at.
-- Read-path only — no wallet mutation.
--
-- 1) Stable clearing origin (earliest credible economic timestamp)
-- 2) Ignore forward-restamped captured_at when ledger credited earlier
-- 3) Pending/eligible use unpaid remainder (amount − active allocations)
-- 4) Active payout allocations only (exclude released/failed items)

BEGIN;

CREATE OR REPLACE FUNCTION public.driver_wallet_captured_at_restamp_suspect(
  p_captured_at timestamptz,
  p_trip_completed_at timestamptz,
  p_ledger_created_at timestamptz
)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT p_captured_at IS NOT NULL
     AND p_trip_completed_at IS NOT NULL
     AND p_ledger_created_at IS NOT NULL
     AND p_captured_at > p_trip_completed_at + interval '48 hours'
     AND p_ledger_created_at < p_captured_at;
$$;

COMMENT ON FUNCTION public.driver_wallet_captured_at_restamp_suspect(timestamptz, timestamptz, timestamptz) IS
  'CAPTURED_AT_RESTAMP_SUSPECT — admin refresh re-stamped captured_at after trip completion and wallet credit. Diagnostic only; clearing must not restart from restamp.';

CREATE OR REPLACE FUNCTION public.driver_wallet_stable_clearing_origin(
  p_captured_at timestamptz,
  p_trip_completed_at timestamptz,
  p_capture_time timestamptz,
  p_ledger_created_at timestamptz,
  p_first_captured_at timestamptz DEFAULT NULL
)
RETURNS timestamptz
LANGUAGE sql
IMMUTABLE
  ...
  SELECT (
    ...
    FROM unnest(
      ARRAY[
        p_first_captured_at,
        CASE
          WHEN NOT public.driver_wallet_captured_at_restamp_suspect(
            p_captured_at, p_trip_completed_at, p_ledger_created_at
          ) THEN p_captured_at
        END,
        p_capture_time,
        p_ledger_created_at,
...
GRANT EXECUTE ON FUNCTION public.driver_wallet_captured_at_restamp_suspect(timestamptz, timestamptz, timestamptz) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.driver_wallet_stable_clearing_origin(timestamptz, timestamptz, timestamptz, timestamptz, timestamptz) TO authenticated, service_role;
