-- MK-260817-006: rematch sets cancelled_by=driver while status stays
-- searching_new_driver. That must not disable pending TTL protection.
-- Terminal cancel is already excluded via status. Isolated; does not mutate MK-260817-006.
...
COMMENT ON FUNCTION public.expire_trip_when_search_exhausted(uuid) IS
  'Expire searching trips after searching_expires_at. Scheduled handover ignores stale searching_expires_at, never uses created_at instant TTL, and stays pending through driver rematch cancelled_by.';

COMMENT ON FUNCTION public.trg_scheduled_handover_block_premature_search_ttl() IS
  'MK-260817-006: while scheduled conversion is pending, do not stamp instant TTL or system-expire the trip. searching_new_driver rematch with cancelled_by=driver stays pending.';
