-- Step 4D.4 corrective migration (LOCAL until explicitly approved).
-- Independently recreates the approved 4C economic-date SSOT from the
-- rolled-back production function state. Does not edit 20260929120000
-- or 20260929130000. Enum-safe purpose/status plus explicit RETURN QUERY
-- casts: production driver_wallet_ledger.amount_pence is integer; RPC
-- contract amount_pence is bigint.
--
-- Does NOT:
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    dwl.id::uuid AS ledger_entry_id,
    dwl.related_trip_id::uuid AS related_trip_id,
    dwl.amount_pence::bigint AS amount_pence,
    r.posting_created_at,
    r.economic_earned_at,
    r.economic_date_status::text AS economic_date_status,
    r.captured_at,
    r.eligible_at,
    r.clearing_status::text AS clearing_status,
    dwl.type::text AS type
...
BEGIN
  ...
  SELECT
    ...
    dwl.amount_pence::bigint AS amount_pence,
    dwl.type::text AS type,
    dwl.description::text AS description,
    r.posting_created_at,
    r.economic_earned_at,
    r.economic_date_status::text AS economic_date_status,
    r.captured_at,
