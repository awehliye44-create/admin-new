-- Enrich get_driver_active_trip_snapshot with driver-safe passenger display fields.
-- Reuses customers.first_name/last_name + get_customer_trip_stats (completed trips only).
-- Does not expose email, phone, or unrelated profile data.
-- Driver-native mapper already reads:
--   passenger_id, passenger_first_name, passenger_last_name,
--   passenger_name, passenger_rating, passenger_trip_count.
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- Driver-safe passenger display: assigned-trip customer only.
