-- Fix pre-existing arity mismatch: dispatch_trip_offers(uuid,text) called
-- maybe_advance_dispatch_after_offer_resolution with 3 args; live signature is (uuid, uuid).
-- No scoring/radius/wave changes. Controlled apply only.
