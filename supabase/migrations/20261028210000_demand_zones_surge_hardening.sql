-- Demand zone hardening: heat-map gate on surge, manual-zone enforcement, settings write lock.

-- ─── resolve_zone_surge: ignore computed zones when heat map is disabled ───
...
BEGIN
  ...
  IF NOT COALESCE(v_settings.heat_map_enabled, true) THEN
    RETURN jsonb_build_object(
      'zone_id', NULL, 'confirmed_demand_level', NULL,
      'applied_multiplier', 1.00, 'surge_enabled', false, 'reason', 'HEAT_MAP_DISABLED');
  END IF;
...
-- ─── admin_save: purge computed zones when heat map is turned off ───
...
BEGIN
  ...
  IF v_existing.heat_map_enabled IS DISTINCT FROM v_new.heat_map_enabled
     AND v_new.heat_map_enabled = false THEN
    DELETE FROM public.driver_demand_zones
    WHERE service_area_id = _service_area_id AND source = 'computed';
  END IF;
...
-- ─── Manual zone CRUD: require manual_zones_enabled on the service area ───
...
CREATE POLICY "Staff manage manual demand zones insert"
  ...
  WITH CHECK (
    source = 'manual'
    AND EXISTS (
      SELECT 1 FROM public.service_area_demand_zone_settings s
      WHERE s.service_area_id = driver_demand_zones.service_area_id
        AND s.manual_zones_enabled = true
    )
    AND (
...
-- ─── Settings: reads via RLS; writes only through admin_save RPC ───

DROP POLICY IF EXISTS "Admins write demand zone settings" ON public.service_area_demand_zone_settings;

REVOKE INSERT, UPDATE, DELETE ON public.service_area_demand_zone_settings FROM authenticated;
