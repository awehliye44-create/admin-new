...
BEGIN
  ...
  IF v_ps.status::text IN ('cancelled','failed','payment_orphaned','orphan_authorisation','RECOVERY_CANCELLED','RECOVERY_DECLINED','RECOVERY_EXPIRED') THEN
  ...
  FOR v_ps IN
    SELECT id, provider_order_id, recovery_attempt_count
     ...
     WHERE payment_provider = 'revolut'
       ...
       AND COALESCE(NULLIF(booking_snapshot, '{}'::jsonb), NULLIF(fare_snapshot, '{}'::jsonb)) IS NOT NULL
    ...
    BEGIN
      UPDATE public.payment_sessions
         SET recovery_attempt_count = recovery_attempt_count + 1,
             ...
             status = CASE WHEN status::text IN ('pending_payment','authorising') THEN 'payment_authorised'::public.payment_session_status ELSE status END,
