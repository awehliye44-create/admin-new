-- MK-260926-004: SQL wave-1 online gate parity with Edge auto-dispatch.
-- Root cause: expire_stale_drivers clears is_online while driver_online_intent
-- stays true (iOS BG GPS gap ~60s). SQL required BOTH flags → silent omit on
-- trip_insert. Edge already accepts is_online OR (intent AND NOT explicit offline).
-- Helper + text-overload rewrite. Do not soften: intent alone must pass when
-- not explicitly offline.

CREATE OR REPLACE FUNCTION public.driver_passes_dispatch_online_availability_gate(
  p_is_online boolean,
  p_driver_online_intent boolean,
  p_offline_reason text DEFAULT NULL
) RETURNS boolean
LANGUAGE sql
STABLE
PARALLEL SAFE
...
AS $gate$
  SELECT
    NOT public.is_explicit_offline_reason(p_offline_reason)
    AND (
      COALESCE(p_is_online, false)
      OR COALESCE(p_driver_online_intent, false)
    );
$gate$;

COMMENT ON FUNCTION public.driver_passes_dispatch_online_availability_gate(boolean, boolean, text) IS
  'MK-260926-004: dispatch online availability — is_online OR intent, fail closed on explicit offline. Parity with Edge auto-dispatch backendAvailabilityOnline.';

REVOKE ALL ON FUNCTION public.driver_passes_dispatch_online_availability_gate(boolean, boolean, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.driver_passes_dispatch_online_availability_gate(boolean, boolean, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.driver_passes_dispatch_online_availability_gate(boolean, boolean, text) TO service_role;
...
BEGIN
  ...
  IF public.is_scheduled_instant_conversion_pending(
       v_trip.dispatch_mode, v_trip.scheduled_status, v_trip.is_scheduled, v_trip.scheduled_at
     ) THEN
    RETURN jsonb_build_object(
  ...
  WITH base AS (
    ...
    WHERE d.approval_status = 'approved' AND d.documents_approved = true
      ...
      -- MK-260926-004: parity with Edge auto-dispatch — intent-online survives
      -- expire_stale clearing is_online (iOS BG GPS gap). Never require BOTH.
      AND public.driver_passes_dispatch_online_availability_gate(
  ...
  SELECT b.*,
    ...
    (COALESCE(b.presence_health, 'healthy') = 'degraded'
      OR public.driver_location_state(true, b.last_heartbeat_at, b.last_gps_sample_at, b.presence_speed) = 'location_frozen'
    ) AS is_degraded,
    -- MK-260904-003: location_frozen is degraded (JS auto-dispatch parity), not a hard reject.
    -- Keep is_frozen for audit context only.
  ...
  SELECT f.*,
    ...
    CASE
      ...
      -- location_frozen intentionally omitted (degraded, still offerable)
  ...
  PERFORM public.log_dispatch_eligibility(
    ...
    jsonb_build_object('wave',v_wave,'round',v_round,'wave_sequence',v_seq,
      ...
      'healthy_heartbeat',s.healthy_heartbeat,'is_frozen',s.is_frozen,'is_idle',s.is_idle,
