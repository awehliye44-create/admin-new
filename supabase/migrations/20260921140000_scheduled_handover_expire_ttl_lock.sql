-- MK-260817-006: keep scheduled handover expire/TTL lock after later-dated
-- migrations that CREATE OR REPLACE expire_trip_when_search_exhausted.
-- Isolated: expire RPC + BEFORE trigger only.
...
BEGIN
  v_pending :=
    lower(COALESCE(NEW.dispatch_mode, '')) = 'scheduled'
...
COMMENT ON FUNCTION public.trg_scheduled_handover_block_premature_search_ttl() IS
  'MK-260817-006: while dispatch_mode=scheduled and conversion is pending, do not stamp instant TTL or system-expire the trip.';
