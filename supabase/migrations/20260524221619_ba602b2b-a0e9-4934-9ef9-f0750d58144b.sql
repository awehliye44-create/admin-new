...
-- ============================================================
ALTER TABLE public.global_dispatch_settings
  DROP COLUMN IF EXISTS shortlist_limit,
  DROP COLUMN IF EXISTS drivers_per_wave,
  DROP COLUMN IF EXISTS wave_delay_seconds,
  DROP COLUMN IF EXISTS dispatch_mode,
  DROP COLUMN IF EXISTS driver_fare_display,
...
--    - one config fetch
--    - admin max_dispatch_rounds replaces hardcoded 3
--    - admin degraded_driver_penalty replaces hardcoded 100
--    - admin presence_max_age_seconds replaces hardcoded 60
--    - locked-driver expiry uses locked_driver_response_minutes only
-- ============================================================
...
BEGIN
  BEGIN
    PERFORM public.dispatch_trip_offers(NEW.id, 'auto');
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[tr_dispatch_trip_offers] dispatch failed for trip %: % (%)',
