-- Calendar expiry vs sticky Admin Approved:
-- 1) Eligibility must read the current document row (is_current), matching SSOT.
-- 2) documents_approved / onboarding_complete only refreshed on document row
--    changes, so calendar expiry left Admin showing Approved while Driver
--    correctly blocked. Refresh nightly + immediately.
...
CREATE OR REPLACE FUNCTION public.refresh_driver_document_approval_flags()
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
COMMENT ON FUNCTION public.refresh_driver_document_approval_flags() IS
  'Recomputes drivers.documents_approved and onboarding_complete from live eligibility so calendar expiry is visible in Admin.';

SELECT public.refresh_driver_document_approval_flags();

DO $cron$
BEGIN
  PERFORM cron.unschedule(j.jobid)
  FROM cron.job j
  WHERE j.jobname = 'refresh-driver-document-approval-flags';
...
SELECT cron.schedule(
  'refresh-driver-document-approval-flags',
  '10 0 * * *',
  $q$SELECT public.refresh_driver_document_approval_flags()$q$
);
