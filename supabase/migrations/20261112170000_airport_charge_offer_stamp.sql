-- Release candidate. Not applied.
-- Version 20261112160000 is already schema_migrations driver_tip_thanks
-- and supabase/migrations/20261112160000_driver_tip_thanks.sql.
-- This version is the next unused slot after that latest applied migration.
--
-- Ordering gap this function cannot close by itself:
-- tr_trips_dispatch_after_insert inserts ride_offers in the trip INSERT.
-- fare_breakdown and airport_charge_pence are written later by booking
-- post-commit enrich. At that instant this function still sees airport 0.
-- auto-dispatch applyAlreadyOfferedAirportRestamp is the defence for that order.
-- Rate stays resolve_wave_commission_percent. No constant airport amount or rate.
-- Rollback: supabase/migrations/rollback/rollback_20261112170000_airport_charge_offer_stamp.sql
