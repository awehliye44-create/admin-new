...
SELECT
  ...
  (COALESCE(tt.card_net_credits, 0) - COALESCE(tt.cash_commission_debits, 0) - GREATEST(COALESCE(lt.total_payouts_sent, 0), COALESCE(pt.payouts_completed, 0)) - COALESCE(lt.total_fees, 0) + COALESCE(lt.adjustments_total, 0)) AS wallet_balance,
  GREATEST(COALESCE(tt.card_net_credits, 0) - COALESCE(tt.cash_commission_debits, 0) - GREATEST(COALESCE(lt.total_payouts_sent, 0), COALESCE(pt.payouts_completed, 0)) - COALESCE(lt.total_fees, 0) + COALESCE(lt.adjustments_total, 0), 0) AS available_for_payout,
  GREATEST(-(COALESCE(tt.card_net_credits, 0) - COALESCE(tt.cash_commission_debits, 0) - GREATEST(COALESCE(lt.total_payouts_sent, 0), COALESCE(pt.payouts_completed, 0)) - COALESCE(lt.total_fees, 0) + COALESCE(lt.adjustments_total, 0)), 0) AS amount_owed_to_onecab
FROM drivers d
