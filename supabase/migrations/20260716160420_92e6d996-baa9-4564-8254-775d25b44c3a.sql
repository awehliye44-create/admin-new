  ...
  FOR v_ps IN
    SELECT ps.id, ps.provider_order_id, ps.recovery_attempt_count
     ...
     WHERE ps.payment_provider = 'revolut'
       ...
       AND ps.status::text NOT IN ('failed','payment_orphaned','orphan_authorisation')
    ...
    BEGIN
      UPDATE public.payment_sessions ps
         SET recovery_attempt_count = ps.recovery_attempt_count + 1,
             ...
             status = CASE WHEN ps.status::text IN ('pending_payment','authorising','cancelled') THEN 'payment_authorised'::public.payment_session_status ELSE ps.status END,
