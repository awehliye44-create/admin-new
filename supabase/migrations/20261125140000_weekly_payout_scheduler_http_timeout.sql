-- pg_net default timeout is 5s. Weekly orchestrator DNS regularly exhausted
-- that budget (HTTP 0ms), so cron 67 "succeeded" without reaching execute.
-- Persist timeout_milliseconds := 60000. Does not invoke scheduler/executor.
-- Does not insert payout/wallet/provider rows.
...
BEGIN
  ...
  BEGIN
    ...
    RAISE LOG '[weekly-payout-orchestrator] edge_invoke_enqueued url=% timeout_ms=60000', v_url;
...
COMMENT ON FUNCTION public.invoke_weekly_payout_scheduler() IS
  'pg_cron: invoke admin-execute-weekly-payout-occurrence. net.http_post timeout 60s. Occurrence idempotency via claim_weekly_payout_occurrence.';
