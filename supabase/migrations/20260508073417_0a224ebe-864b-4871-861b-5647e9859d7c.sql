...
BEGIN
  ...
  SELECT
    COUNT(*) FILTER (WHERE bdl.phase = 'booking_sent'),
    COUNT(*) FILTER (WHERE bdl.phase = 'booking_received'),
    COUNT(*) FILTER (WHERE bdl.phase = 'push_enqueued'),
    COUNT(*) FILTER (WHERE bdl.phase = 'push_sent')
  INTO v_offered, v_received, v_push_enqueued, v_push_sent
  FROM bdl;

  SELECT
    COUNT(*),
    COUNT(*) FILTER (WHERE ro.broadcast_round > 1 OR ro.revoked_reason IN ('reassigned','ack_timeout')),
  ...
  FROM (
    SELECT date_trunc('hour', b.created_at) AS bucket,
      COUNT(*) FILTER (WHERE b.phase = 'booking_sent') AS offered,
      COUNT(*) FILTER (WHERE b.phase = 'booking_received') AS received
    FROM public.booking_delivery_log b
    LEFT JOIN public.trips t ON t.id = b.booking_id
    WHERE b.created_at >= p_start
      AND b.created_at < p_end
  ...
  FROM (
    SELECT date_trunc('hour', b.created_at) AS bucket,
      COUNT(*) FILTER (WHERE b.phase = 'ack_timeout') AS timeout_count,
      COUNT(*) FILTER (WHERE b.phase IN ('reassigned','reassigned_auto_dispatch')) AS reassigned_count
    FROM public.booking_delivery_log b
    LEFT JOIN public.trips t ON t.id = b.booking_id
    WHERE b.created_at >= p_start
      AND b.created_at < p_end
