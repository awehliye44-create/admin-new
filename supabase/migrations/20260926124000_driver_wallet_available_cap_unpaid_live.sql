-- Cap Available by unpaid live after settlement Pending.
-- Already-paid cleared history still present in eligible_sum must not make
...
BEGIN
  ...
  -- Unpaid invariant: Available + Pending = Live (debt/reservations aside).
  -- Cap cleared earnings by live − pending so already-paid history cannot inflate Available.
