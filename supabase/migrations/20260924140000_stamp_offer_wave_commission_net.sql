-- Offer insert must stamp driver net from Admin per-wave commission, not
-- a flat 15% preview and not a previous-wave reduction pin.
--
-- Admin Auto-Dispatch Rules (MK-260815-008):
--   base 15% · W1 reduction 15pp → 0% · W2 12pp → 3% · W3 9pp → 6%
-- Previous stamp used a flat 15% preview so every wave stored 421 on a 495 fare.
-- Later waves were also pinned to W1's 15pp, so W2/W3 never applied.
--
-- Floor is 0 here: each wave uses its configured reduction from
-- global_dispatch_settings. Do not rewrite historical offer rows.
