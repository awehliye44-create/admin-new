-- Expose trips.special_instructions on get_driver_active_trip_snapshot.
-- Presentation-only passthrough for Driver pickup notes (Customer/Corporate).
-- No money movement, assignment, or lifecycle behaviour change.
...
BEGIN
  ...
  IF FOUND THEN
    ...
    v_active := jsonb_build_object(
      ...
      -- Pickup note SSOT — trips.special_instructions (Customer/Corporate).
      'special_instructions', NULLIF(left(btrim(COALESCE(v_trip.special_instructions::text, '')), 1000), '')
    );
  ELSE
    v_active := NULL;
  END IF;
