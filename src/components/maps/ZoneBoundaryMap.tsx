      ...
      try {
        const { map, detachResize: detach } = await createMapboxMap({
          ...
          onLoad: (m) => {
            ...
            m.addSource(ZONE_SRC, { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
            m.addLayer({ id: ZONE_FILL, type: 'fill', source: ZONE_SRC, paint: { 'fill-color': color, 'fill-opacity': 0.25 } });
            m.addLayer({ id: ZONE_LINE, type: 'line', source: ZONE_SRC, paint: { 'line-color': color, 'line-width': 2 } });
