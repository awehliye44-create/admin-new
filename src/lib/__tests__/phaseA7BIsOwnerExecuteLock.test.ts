/**
 * Lock: Phase A7B is_owner EXECUTE draft.
...
const CANONICAL = "supabase/migrations/20261109070000_phase_a7b_is_owner_execute_lock.sql";
const ROLLBACK = "supabase/migrations/rollback/rollback_20261109070000_phase_a7b_is_owner_execute_lock.sql";
const SIG = "is_owner(uuid)";
...
describe("phaseA7BIsOwnerExecuteLock", () => {
  it("revokes authenticated and keeps service_role for the proven Edge caller", () => {
    ...
    expect(sql).not.toMatch(/REVOKE ALL ON FUNCTION public\.is_owner\(uuid\) FROM service_role/i);
    expect(sql).not.toMatch(/REVOKE ALL ON FUNCTION public\.(has_role|has_corporate_access|can_write_corporate)/i);
