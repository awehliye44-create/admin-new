/**
 * Lock: Phase A8B2 upsert_pending_customer_signup EXECUTE ACL draft.
...
const CANONICAL =
  "supabase/migrations/20261109130000_phase_a8b2_upsert_pending_customer_signup_execute_lock.sql";
const ROLLBACK =
  "supabase/migrations/rollback/rollback_20261109130000_phase_a8b2_upsert_pending_customer_signup_execute_lock.sql";
const SIG =
  "public.upsert_pending_customer_signup\\(uuid, text, text, text, text, text\\)";

function read(rel: string): string {
  return fs.readFileSync(path.join(ROOT, rel), "utf8");
}

describe("phaseA8B2UpsertPendingCustomerSignupExecuteLock", () => {
  it("revokes authenticated EXECUTE and retains service_role without body changes", () => {
    ...
    expect(rb).not.toMatch(/PERFORM public\.upsert_pending_customer_signup/i);
  });
});
