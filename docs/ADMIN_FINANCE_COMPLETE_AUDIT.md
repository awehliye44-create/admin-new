...
| Wallet `wallet_status` / UI freeze banner | Credit variance vs payout eligibility (separate) | Mixed: `payout_blocked`/`payouts_enabled` forced FROZEN; UI says “wallet mismatch” | MK0006 credit OK but FROZEN + mismatch copy | Credit OK; payout shows exact block reason (e.g. payouts disabled / restricted) | `fetchDriverWalletPayoutSnapshot` + `DriverWalletOverviewCards` |
